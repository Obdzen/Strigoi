...
pub fn report_syntax_errors(
    path: &Path,
    mut file_cache: &mut FileCache,
    errs: Vec<Simple<String>>,
) {
            ...
            chumsky::error::SimpleReason::Unclosed { span, delimiter } => report
                ...
                .with_label(
                    unclosed_delim_label(path, span.clone(), delimiter)
                        .with_color(crate::colors::PURPLE),
                )
                .with_label(unclosed_delim_label_2(path, &e)),
            chumsky::error::SimpleReason::Unexpected => report
                .with_message(unexpected_msg(&e))
                .with_label(unexpected_label(path, &e).with_color(crate::colors::PURPLE)),
            chumsky::error::SimpleReason::Custom(msg) => report.with_message(msg).with_label(
                custom_error_label(path, e.span(), msg).with_color(crate::colors::PURPLE),
            ),
        };
